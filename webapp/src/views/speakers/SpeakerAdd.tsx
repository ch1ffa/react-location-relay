...
export const SpeakerAdd: FC<ISpeakerAdd> = ({ id, isOpen, closeModal }) => {
  ...
  return (
    <Modal isOpen={isOpen} onDismiss={closeModal}>
      <div className={tw`flex flex-col gap-4 w-[200] h-[200] `}>
        <div>
          Add Speaker
        </div>
        <input
        ...
        <div className={tw`flex justify-end`}>
          <button
            className={tw`py-1 px-2 ml-2 text-xs bg-white hover:bg-gray-100 rounded`}
            onClick={closeModal}
          >
            Cancel
          </button>
          <button className={tw`py-1 px-2 ml-2 text-xs bg-blue-500 hover:bg-blue-400 text-white rounded disabled:bg-blue-200`}
